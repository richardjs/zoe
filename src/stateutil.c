...
{
    return !(piece->type == other->type && piece->coords.q == other->coords.q && piece->coords.r == other->coords.r && piece->player == other->player && (bool)piece->on_top == (bool)other->on_top && !(piece->on_top && Piece_compare(piece->on_top, other->on_top)));
}
    ...
    bool debug_print)
{
    ...
    for (int p = 0; p < NUM_PLAYERS; p++) {
        if (state->piece_count[p] != other->piece_count[p]) {
            if (debug_print)
                fprintf(stderr, "Different piece_count\n");
            return true;
        }
        ...
        for (int i = 0; i < state->piece_count[p]; i++) {
            if (Piece_compare(&state->pieces[p][i], &other->pieces[p][i])) {
                if (debug_print)
                    fprintf(stderr, "Different pieces\n");
                return true;
            }
        }
    }
    ...
    for (int q = 0; q < GRID_SIZE; q++) {
        for (int r = 0; r < GRID_SIZE; r++) {
            if (state->grid[q][r] == NULL) {
                ...
                if (debug_print)
                    fprintf(stderr, "Different grid NULLs\n");
                return true;
            }

            if (Piece_compare(state->grid[q][r], other->grid[q][r])) {
                if (debug_print)
                    fprintf(stderr, "Different grid\n");
                return true;
            }
            ...
            for (int p = 0; p < NUM_PLAYERS; p++) {
                if (state->neighbor_count[p][q][r] != other->neighbor_count[p][q][r]) {
                    if (debug_print)
                        fprintf(stderr, "Different neighbor_count\n");
                    return true;
                }
            }
        }
