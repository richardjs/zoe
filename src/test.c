...
uint_fast8_t State_articulation_points(const struct State *state,
...
int main(int argc, char *argv[]) {
  ...
  // State normalizaiton
  {
    ...
    if (p1_piece->coords.q != 2 || p2_piece->coords.r != 1) {
      printf("q not normalized: %d %d\n", p1_piece->coords.q,
             p2_piece->coords.q);
    }
    if (p1_piece->coords.r != 1 || p2_piece->coords.r != 1) {
      printf("r not normalized: %d %d\n", p1_piece->coords.r,
             p2_piece->coords.r);
    }
  }
  ...
  // Hands derivation
  {
    ...
    if (state.hands[P1][QUEEN_BEE] != 0 || state.hands[P2][SPIDER] != 1 ||
        state.hands[P1][ANT] != 3 || state.hands[P2][ANT] != 2) {
  ...
  // Articulation points
  {
    ...
    for (int q = 0; q < GRID_SIZE; q++) {
      for (int r = 0; r < GRID_SIZE; r++) {
        if ((state.cut_points[q][r] && (q != 0 && r != 1)) ||
  ...
  // Grasshopper moves
  {
    ...
    for (int i = 0; i < state.action_count; i++) {
      ...
      if (!((action->to.q == 2 && action->to.r == 2) ||
  ...
  // Spider moves
  {
    ...
    for (int i = 0; i < state.action_count; i++) {
      ...
      if (!((action->to.q == 2 && action->to.r == 3) ||
  ...
  // Beetle moves
  {
    ...
    for (int i = 0; i < state.action_count; i++) {
      ...
      struct Piece *piece =
          State_top_piece(&state, action->from.q, action->from.r);
      if (piece->type != BEETLE)
        continue;
  ...
  // Queen must be placed by turn 4
  {
    ...
    for (int i = 0; i < state.action_count; i++) {
      if (state.actions[i].from.q != PLACE_ACTION &&
          state.actions[i].from.r != QUEEN_BEE) {
  ...
  // FTM rule for beetles on top of hive
  {
    ...
    for (int i = 0; i < state.action_count; i++) {
      struct Action *action = &state.actions[i];
      if (action->from.q == 2 && action->from.r == 1 && action->to.q == 2 &&
          action->to.r == 2) {
