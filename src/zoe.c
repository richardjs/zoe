...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < state.action_count; i++) {
    ...
    for (int j = 0; j < TOP_ACTIONS; j++) {
      ...
      float s = -1 * results.nodes[top_actionis[j]].value /
                results.nodes[top_actionis[j]].visits;
      if (score > s) {
  ...
  for (int i = 1; i < TOP_ACTIONS; i++) {
    ...
    float score = -1 * results.nodes[top_actionis[i]].value /
                  results.nodes[top_actionis[i]].visits;
