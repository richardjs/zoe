...
bool Coords_calc_adjacent(const struct Coords *coords,
                          const struct Coords *other) {
...
bool Coords_map_adjacent(const struct Coords *coords,
                         const struct Coords *other) {
    ...
    for (int r = 0; r < GRID_SIZE; r++) {
      ...
      for (int oq = 0; oq < GRID_SIZE; oq++) {
        for (int or = 0; or < GRID_SIZE; or ++) {
          ...
          other.q = oq;
          other.r = or ;
          coords_adjacent_map[q][r][oq][or] =
              Coords_calc_adjacent(&coords, &other);
        }
      }
    }
  }
